...
dmp = galf.get_dark_matter_parameters(Mvir)
sdp = galf.get_stellar_disc_parameters(Mvir)
rhos = galf.get_densities(Mvir, grid.r, grid.theta, splashback_cutoff=True, total=True)
...
# load sym field screening radius
rs_Ms = {}
rs_Ms_SSB = {}
...
rhoratio = rhos['total'] / rho_m
rhoratio[rhoratio == 0] = None
logrho = np.log10(rhoratio)

### plottting ###
# 5% extra so the plot fills the full semicircle
...
text_Ms2 = r'$[\log_{{10}}(L_c/\mathrm{{kpc}}) = {:.1f}]$'.format(logLc_fixed)
text_Lc1 = r'$\log_{10}(L_c/\mathrm{kpc})$'
text_Lc2 = r'$[\log_{{10}}(M_\mathrm{{sym}}/M_\mathrm{{pl}}) = {:.1f}]$'.format(logMs_fixed)
text_fRrow = r'$f(R)$'
text_symrow = 'Symmetron'
axfRT.text(0.5, 0.5, s=text_fR, ha='center')
axMsT_text1 = axMsT.text(0.27, 0.5, text_Ms1, ha='center')
axMsT_text2 = axMsT.text(0.74, 0.5, text_Ms2, ha='center', fontsize=10)
axLcT_text1 = axLcT.text(0.27, 0.5, text_Lc1, ha='center')
axLcT_text2 = axLcT.text(0.74, 0.5, text_Lc2, ha='center', fontsize=10)
...
for ind, ax in enumerate(axs):
    ...
    for x, y in rss[param_ind].items():
        ...
        if np.min(y) <= rmaxplotted[LR_ind] / kpc \
            ...
            # extend rs to cover full 180 degrees
            ye_approx = np.append(y_approx[-1], y_approx)
            ye_approx = np.append(ye_approx, y_approx[0])
